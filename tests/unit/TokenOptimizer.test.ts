import { TokenOptimizer, TokenOptimizerConfig, OptimizationResult } from '../../src/optimization/TokenOptimizer.js';
import { SearchResult } from '../../src/core/VectorStore.js';
...
`;
      ...
      const compressed = (optimizer as any).removeComments(code);
...
`;
      ...
      const simplified = (optimizer as any).simplifyWhitespace(code);
    ...
    it('should intelligently truncate long functions', () => {
      ...
      const truncated = (optimizer as any).intelligentTruncate(longCode, 200);
  ...
  describe('Token Estimation', () => {
    it('should estimate tokens approximately', () => {
      ...
      const shortTokens = (optimizer as any).estimateTokens(shortText);
      const longTokens = (optimizer as any).estimateTokens(longText);
  ...
  describe('Chunk Prioritization', () => {
    it('should prioritize by relevance first', () => {
      ...
      const prioritized = (optimizer as any).prioritizeChunks(chunks);
  ...
  describe('Configuration Edge Cases', () => {
    it('should handle extreme configuration values', () => {
      const extremeConfig: Partial<TokenOptimizerConfig> = {
        maxTokens: 1,
  ...
  describe('Large Dataset Handling', () => {
    ...
    it('should handle very large individual results', () => {
      const largeContent = Array.from({ length: 1000 }, (_, i) => 
        `  const variable${i} = "This is a very long line of code with lots of content";`
      ).join('\n');
...
`;
  ...
  describe('Error Handling and Edge Cases', () => {
    it('should handle null and undefined query gracefully', () => {
      ...
      expect(() => optimizer.optimizeSearchResults(results, null as any)).not.toThrow();
      expect(() => optimizer.optimizeSearchResults(results, undefined as any)).not.toThrow();
      expect(() => optimizer.optimizeSearchResults(results, '')).not.toThrow();
    });

    it('should handle results with malformed data', () => {
      const malformedResults = [
        { ...createMockResult(), content: null } as any,
        { ...createMockResult(), name: undefined } as any,
        { ...createMockResult(), relevance: 'invalid' } as any,
...
`;
  ...
  describe('Memory and Cache Management', () => {
    ...
    it('should handle memory-intensive operations', () => {
      const memoryIntensiveResults = Array.from({ length: 100 }, (_, i) => 
        createMockResult({
  ...
  describe('Token Estimation Accuracy', () => {
    it('should provide reasonable token estimates', () => {
      ...
      testCases.forEach(({ text, expectedRange }) => {
        const tokens = (optimizer as any).estimateTokens(text);
    ...
    it('should handle token estimation for various languages', () => {
      ...
      Object.entries(codeExamples).forEach(([language, code]) => {
        const tokens = (optimizer as any).estimateTokens(code);
